...
exports.findIgnoredFolderSettings = findIgnoredFolderSettings;
exports.warnAboutIgnoredSettings = warnAboutIgnoredSettings;
 ...
 */
function findIgnoredFolderSettings() {
    ...
    for (const folder of folders) {
        ...
        for (const key of WATCHED) {
            ...
            if (folderValue === undefined || isEmpty(folderValue))
                continue;
 ...
 */
async function warnAboutIgnoredSettings(context) {
    ...
    const message = `Script Progress: ${keys.map(k => `"${k}"`).join(', ')} ${keys.length === 1 ? 'is' : 'are'} set in ` +
