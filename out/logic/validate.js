...
exports.problemsFor = problemsFor;
exports.problemText = problemText;
...
function validateSettings(raw) {
    ...
    out.push(...validateArray(raw.buttons, 'quickActions', 'quickActions.buttons', (b, add) => {
        ...
        if (!label)
            add('needs a "label" — it is the text on the button.');
        if (!str(b.command))
            add(`needs a "command" — the shell command to run.`, label);
        if (b.confirm !== undefined && typeof b.confirm !== 'boolean')
            add('"confirm" must be true or false.', label);
        if (b.icon !== undefined && !str(b.icon))
            add('"icon" must be a codicon name, e.g. "play".', label);
        const when = b.enableWhen;
        if (when !== undefined) {
            if (!isObject(when))
                add('"enableWhen" must be an object like { "metric": "issues", "gt": 0 }.', label);
            else {
                if (!str(when.metric))
                    add('"enableWhen" needs a "metric" to look at.', label);
                ...
                if (when.eq === undefined && !comparators.length)
                    add('"enableWhen" has no comparison, so it never disables the button.', label);
            }
        }
        return label;
    }));
    ...
    const knownNames = new Set((Array.isArray(raw.processes) ? raw.processes : [])
        .map(p => (isObject(p) ? str(p.name) : ''))
        .filter(Boolean)
    ...
    out.push(...validateArray(raw.processes, 'processCalendar', 'processCalendar.processes', (p, add) => {
        ...
        if (!str(p.name))
            add('needs a "name" — it is matched against the task name your script reports.');
        const freq = str(p.frequency);
        if (!freq)
            add('needs a "frequency" of "monthly", "weekly" or "daily".', label);
        else if (!FREQUENCIES.includes(freq))
            add(`has frequency "${freq}"; expected "monthly", "weekly" or "daily".`, label);
        if (freq === 'monthly') {
            if (p.dayOfMonth === undefined)
                add('is monthly but has no "dayOfMonth", so it can never be overdue.', label);
            else if (!isInt(p.dayOfMonth, 1, 31))
                add(`has dayOfMonth ${fmt(p.dayOfMonth)}; expected a whole number from 1 to 31.`, label);
        }
        ...
        if (p.dueHour !== undefined && !isInt(p.dueHour, 0, 23))
            add(`has dueHour ${fmt(p.dueHour)}; expected 0 to 23.`, label);
        ...
        if (p.dependsOn !== undefined) {
            if (!Array.isArray(p.dependsOn))
                add(`has a "dependsOn" that is ${fmt(p.dependsOn)}; expected a list of process names.`, label);
            else
                for (const dep of p.dependsOn) {
                    if (typeof dep !== 'string' || !dep.trim()) {
                        add(`has a "dependsOn" entry that is ${fmt(dep)}; expected a process name.`, label);
                        continue;
                    }
    ...
    for (const key of thresholds) {
        const t = raw.deltaThresholds[key];
        if (!isObject(t)) {
            out.push({ area: 'deltaTracker', label: key, message: 'threshold must be an object like { "min": -5, "max": 5 }.' });
            continue;
        }
        const min = t.min, max = t.max;
        if (min !== undefined && typeof min !== 'number')
            out.push({ area: 'deltaTracker', label: key, message: '"min" must be a number.' });
        if (max !== undefined && typeof max !== 'number')
            out.push({ area: 'deltaTracker', label: key, message: '"max" must be a number.' });
 ...
 */
function problemsFor(problems, area) {
...
function validateArray(value, area, key, check) {
    const out = [];
