...
function complianceReport(process, history, now, count = 12) {
    ...
    const attempts = history
        .filter(r => (0, calendar_1.matchesProcess)(r.task, process))
...
function coverage(calendar, history, metricsOutOfRange, metricsTracked, now, days = 30, weights = exports.DEFAULT_COVERAGE_WEIGHTS, historyCap = 0) {
    ...
    if (recent.length) {
        ...
        const oldest = history.reduce((min, r) => Math.min(min, (0, time_1.parseIso)(r.date)?.getTime() ?? Infinity), Infinity);
    ...
    if (!inputs.length || !observed)
        return { percent: null, inputs, reason: 'nothing-measured' };
    ...
    if (!(total > 0))
        return { percent: null, inputs, reason: 'no-weight' };
    const score = inputs.reduce((n, i) => n + clamp01(i.score) * i.weight, 0) / total;
    if (!Number.isFinite(score))
        return { percent: null, inputs, reason: 'not-a-number' };
