...
exports.comparisonText = comparisonText;
...
function comparisonText(c) {
    ...
    L.push(`|---|---|---|`);
    L.push(`| Run | ${(0, time_1.dateTime)(a.date)} | ${(0, time_1.dateTime)(b.date)} |`);
    L.push(`| Outcome | ${a.success ? 'OK' : 'FAILED'}${a.category ? ` (${a.category})` : ''} | ${b.success ? 'OK' : 'FAILED'}${b.category ? ` (${b.category})` : ''} |`);
    L.push(`| Duration | ${(0, time_1.formatDuration)(Number(a.elapsed) || 0)} | ${(0, time_1.formatDuration)(Number(b.elapsed) || 0)}${c.durationDelta ? ` — ${c.durationDelta > 0 ? 'slower' : 'faster'} by ${(0, time_1.formatDuration)(Math.abs(c.durationDelta))}${pct(c.durationPct)}` : ''} |`);
    L.push(`| Warnings | ${a.warnings ?? 0} | ${b.warnings ?? 0} |`);
    L.push('');
    if (!c.bIsNewer)
        L.push('> Note: the compared run is the OLDER of the two, so "changed" reads backwards in time.');
    if (!c.sameTask)
        L.push('> Note: these are different scripts, so metrics with the same name may not mean the same thing.');
    if (!c.bIsNewer || !c.sameTask)
        L.push('');
    if (c.outcomeChanged) {
    ...
    if (!w.added.length && !w.resolved.length && !w.unchanged.length) {
        ...
        if (w.added.length) {
            L.push(`**New (${w.added.length})**`);
            L.push('');
            for (const m of w.added)
                L.push(`- ${m}`);
            L.push('');
        }
        if (w.resolved.length) {
            L.push(`**Gone (${w.resolved.length})**`);
            L.push('');
            for (const m of w.resolved)
                L.push(`- ${m}`);
            L.push('');
        }
        if (w.unchanged.length) {
            L.push(`**Still there (${w.unchanged.length})**`);
            L.push('');
            for (const m of w.unchanged)
                L.push(`- ${m}`);
            L.push('');
        }
    }
    if (c.touchedAdded.length || c.touchedRemoved.length) {
        L.push('## Touched');
        L.push('');
        for (const id of c.touchedAdded)
            L.push(`- \`+\` ${id}`);
        for (const id of c.touchedRemoved)
            L.push(`- \`−\` ${id} (not this run)`);
        L.push('');
    }
    return L.join('\n').replace(/\n{3,}/g, '\n\n').trimEnd() + '\n';
}
