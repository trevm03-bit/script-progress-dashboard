...
function digestHtml(data, settings, now, days = 7) {
    ...
    const runs = data.history
        .filter(r => { const d = (0, time_1.parseIso)(r.date); return !!d && d >= from && d.getTime() <= now.getTime(); })
    ...
    const day = (d) => `${d.getDate()} ${['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][d.getMonth()]}`;
    const P = [];
    ...
    const cov = (0, summary_1.coverageFor)(data, settings, now);
    ...
    if (overdue.length || blocked.length) {
        ...
        if (overdue.length)
            P.push(`<b style="color:${BAD}">Overdue:</b> ${esc(overdue.map(r => r.process.label || r.process.name).join(', '))}<br>`);
        if (blocked.length)
            P.push(`<b style="color:${MUTED}">Waiting on something upstream:</b> ${esc(blocked.map(r => `${r.process.label || r.process.name} (${r.note})`).join('; '))}`);
        P.push(`</p>`);
    }
    const actions = (0, compliance_1.pendingActions)(data.history, now, settings.pendingActions.maxAgeDays);
    if (actions.length) {
    ...
    if (byTask.size) {
        ...
        <td style="padding:5px 8px;border-bottom:1px solid ${LINE};color:${MUTED}">${esc((0, time_1.formatDuration)(avg))}</td>
      </tr>`);
        }
        P.push(`</table>`);
    }
    ...
    if (impact.length) {
        ...
        for (const t of impact) {
            ...
            P.push(`<li style="margin:2px 0">${esc(t.label)}: <b>${esc((0, sparkline_1.formatMetric)(t.total, fmt))}</b> total, ${esc((0, sparkline_1.formatMetric)(t.thisMonth, fmt))} this month <span style="color:${MUTED}">(${t.runs} runs)</span></li>`);
        }
        P.push(`</ul>`);
    }
    if (failed.length) {
        ...
        if (pattern)
            P.push(`<p style="margin:0 0 12px;color:${WARN}"><b>Pattern:</b> ${esc(pattern)}</p>`);
    }
...
//# sourceMappingURL=digestHtml.js.map