 ...
 */
function sortableTh(col: number, label: string, sorted?: 'ascending' | 'descending'): string {
...
function historyRow(r: RunRecord, settings: Settings, all: RunRecord[], verdict: DurationVerdict | undefined, identity: boolean | undefined): string {
  ...
  const main = `<tr class="${r.success ? '' : 'row-failed'}${expandable ? ' expandable' : ''}"${rowAttrs} data-hay="${esc(hay)}" data-kinds="${kinds}">
  ...
  if (Array.isArray(r.warningItems) && r.warningItems.length) parts.push(`<div class="detail-block"><div class="detail-h">Warnings</div>${r.warningItems.map(w => `<div class="warning-card"><span class="warning-time">${esc(clockTime(w.time))}</span> ${esc(w.msg)}</div>`).join('')}</div>`);
