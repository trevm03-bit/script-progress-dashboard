...
export function reportHtml(data: DashboardData, settings: Settings, now: Date, title = 'Script Progress report'): string {
  const body = renderSections(data, { ...settings, dashboard: { ...settings.dashboard, collapsible: false }, sections: { ...settings.sections, quickActions: false } }, { now, surface: 'panel', trusted: false, collapsed: [] });
